...
public class BasePage {
	...
	private  static HashMap<String, Object> dp = new HashMap<String, Object>();
	...
	public  Boolean waitForElementToBeDisplayed(@NonNull WebElement element) {
		Boolean elementDispalyed = new FluentWait<>(driver)
				.withTimeout(Duration.ofSeconds(60))
				.pollingEvery(Duration.ofSeconds(1))
				.ignoring(NoSuchElementException.class)
				.until(driver -> element.isDisplayed());
		return elementDispalyed;
	}
	...
	//Used generic to instantiate the page class
	...
	//Gettter and setter for Hashmap
	public void setDp(String key, Object value ) {
		dp.put(key, value);
	}

	public Object getDp(String key){
		return dp.get(key);
	}

	//Page title
	public <P extends BasePage> P pageTitle(Class<P> pageclass) {
		setDp(pageclass.getSimpleName().concat("Title"), driver.getTitle());
		try {
			return pageclass.getDeclaredConstructor(WebDriver.class).newInstance(driver);
		} catch (Exception e) {
			e.printStackTrace();
	...
	//Page URL
	public <P extends BasePage> P currentUrl(Class<P> pageclass) {
		waitForPage();
		setDp(pageclass.getSimpleName().concat("Url"), driver.getCurrentUrl());
		try {
			return pageclass.getDeclaredConstructor(WebDriver.class).newInstance(driver);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String dpValues(String key) {
		
		return getDp(key).toString();		
	}
	
	//Getter for HashMap
	public static HashMap<String, Object> getDp() {
		return dp;
	}
